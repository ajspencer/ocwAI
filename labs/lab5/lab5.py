...
def most_misclassified(classifier, n=5):
    ...
    """
    #rip readable code
    return [legislator_info(x[0]) for x in (sorted(zip(classifier.data, classifier.data_weights), key=lambda x: x[1], reverse=True))[0:5] ]
...
classifiers_for_best_ensemble = ['dt', 'knn', 'svml',
                                 'svmp3']
...
WHAT_I_FOUND_INTERESTING = 'Writing the neural net'
WHAT_I_FOUND_BORING = 'playing with orange'
