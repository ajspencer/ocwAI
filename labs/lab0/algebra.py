...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    '*' will not help you.
    """
    expr1 = ensureSimple(expr1)
    expr2 = ensureSimple(expr2)
    answer = []
    #at the point the product will only be of terms
    for firstExpr in expr1:
        for secondExpr in expr2:
            answer.append(Product([firstExpr, secondExpr]))
    return Product(answer)
    raise NotImplementedError

def ensureSimple(expr):
    newExpr = []
    for term in expr:
        if( ( isinstance(term, Product) and len(term) > 2) or isinstance(term, Sum)):
            newItem = term.simplify()
            newExpr.append(newItem)
        else:
            newExpr.append(term)
    return newExpr
p = Product([2, Sum(['x', 1])])
