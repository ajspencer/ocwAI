...
# family_rules.
same = IF( OR('male (?x)',
                'female (?x)'), THEN('same-person (?x) (?x)'))
son = IF( AND('parent (?x) (?y)',
             'male (?y)'), THEN('son (?y) (?x)'));
daughter = IF( AND('parent (?x) (?y)',
             'female (?y)'), THEN('daughter (?y) (?x)'));
father = IF( AND('parent (?x) (?y)',
             'male (?x)'), THEN('father (?x) (?y)'));
mother = IF( AND('parent (?x) (?y)',
             'female (?x)'), THEN('mother (?x) (?y)'));
sister = IF( AND('parent (?x) (?y)',
             'parent (?x) (?z)',
             'female (?z)', NOT('same-person (?z) (?y)')),
             THEN('sister (?z) (?y)'));
brother = IF( AND('parent (?x) (?y)',
             'parent (?x) (?z)',
             'male (?z)', NOT('same-person (?z) (?y)')),
             THEN('brother (?z) (?y)'));
cousin = IF( AND('parent (?x) (?y)',
             OR('brother (?x) (?z)', 'sister (?x) (?z)'),
             'parent (?z) (?a)'),
             THEN('cousin (?y) (?a)', 'counsin (?a) (?y)' ));
grand = IF(AND('parent (?x) (?y)',
               'parent (?z) (?x)'), THEN('grandparent (?z) (?y)', 'grandchild (?y) (?z)'));
family_rules = [same, son, daughter, father, mother, sister, brother, cousin, grand]                    # fill me in
